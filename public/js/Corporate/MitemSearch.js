...
function totalF() {
  //When the page is first loaded it takes out the Total amount
...
function autocompleteEventListenerC() {
  let clientName = document.querySelector("#clientName"); //search for the input

  clientName.addEventListener("input", (event) => {
    //when data is entered
    axios
      .post(`https://maxfood.up.railway.app/axiosautocomplete1`, {
        //it send it to this route which is in index
...
function autocompleteCodeC(id, autocomplete) {
  let li = document.createElement("li"); // create a list and add the autocomplete code
...
function addAutocompleteSingleC(autocomplete) {
  ...
  let id = makeidC(6); //randon id is made
  let code = autocompleteCodeC(id, autocomplete); //since it returns only li
  ...
  selected.addEventListener("click", (e) => {
    ...
    let select = document.querySelector("#selectRoute");
    ...
    axios
      ...
      .then((res) => {
        ...
        select.value = specialPriceArray.route;
        select.innerText = specialPriceArray.route;
      });
    ...
    .addEventListener("input", (event) => {
      if (event.target.classList.contains("searchInput")) {
        ...
        selected.addEventListener("click", (e) => {
          ...
          spriceArray = specialPriceArray.specialPrice;
          for (let key in spriceArray) {
            if (spriceArray[key].itemId === idValue) {
              price.value = spriceArray[key].price;
...
let specialPriceEdit = async function () {
  window.addEventListener("load", function (e) {
    let editbtn = document.querySelector("#saveButton");
