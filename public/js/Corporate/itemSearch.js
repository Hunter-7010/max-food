...
function fetchAutocomplete() {
  ...
  axios
    .post(`https://maxfood.up.railway.app/axiosautocomplete`, { val: inputVal })
    .then((res) => {
...
function useArrayAddAutoComplete(array) {
  ...
  for (let i = 0; i < arrayData.length; i++) {
    const title = arrayData[i].itemName;
...
function createUrlSearchQuery(searchValToPut) {
  let url = `https://maxfood.up.railway.app/items?val=${searchValToPut}`;
  return url;
}
